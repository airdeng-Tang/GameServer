    ...
    class TeamService : Singleton<TeamService>
    {
        ...
        void OnTeamInviteRequest(NetConnection<NetSession> sender, TeamInviteRequest request)
        {
            ...
            if(target.Session.Character.Team != null)
            {
                sender.Session.Response.teamInviteRes = new TeamInviteResponse();
                sender.Session.Response.teamInviteRes.Result = Result.Failed;
                sender.Session.Response.teamInviteRes.Errormsg = "对方已加入其他队伍";
                sender.SendResponse();
                return;
            }
            //转发请求
        ...
        void OnTeamLeave(NetConnection<NetSession> sender, TeamLeaveRequest request)
        {
            ...
            //var member = SessionManager.Instance.GetSession(request.characterId);
            //character.Team.Leave(member.Session.Character);

            TeamManager.Instance.LeaveTeamByCharacterInList(character);//删除角色
            //foreach(var t in t1.Members)
            //{
            //    SessionManager.Instance.GetSession(t.Id).Session.Character.Team = 
            //}

            //member.Session.Character.Team = null;
            sender.SendResponse();
        }
    }
}
