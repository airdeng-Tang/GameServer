...
namespace GameServer.Models
{
    class Guild
    {
        ...
        internal bool JoinAppove(NGuildApplyInfo apply)
        {
            ...
            TCharacter character1 = DBService.Instance.Entities.Characters.FirstOrDefault(v => v.ID == apply.characterId);
            character1.GuildId = apply.GuildId;
            DBService.Instance.Save();
        ...
        public void AddMember(int id,string name,int @class,int level,GuildTitle title)
        {
            ...
            else
            {
                ...
                TCharacter dbChar = DBService.Instance.Entities.Characters.SingleOrDefault(v => v.ID == id);
        ...
        public void Leave(Character member)
        {
            ...
            {
                Log.InfoFormat("{0}号公会的sb会长想自己退出自己公会, 真为他的团员感到痛心",this.Id);
                return;
            }

            var removeMember = this.Data.Members.FirstOrDefault(v => v.Id == member.Id);
            DBService.Instance.Entities.TGuildMembers.Remove(removeMember);
            TCharacter character = DBService.Instance.Entities.Characters.FirstOrDefault(v => v.ID == member.Id);
        ...
        internal void ExecuteAdmin(GuildAdminCommand command,int targetId, int sourceId)
        {
            ...
            switch (command)
            {
                ...
                case GuildAdminCommand.Depost:
                    ...
                    break;
                case GuildAdminCommand.Transfet:
                    target.Title = (int)GuildTitle.President;
                    source.Title = (int)GuildTitle.None;
                    this.Data.LeaderID = targetId;
                    //this.Leader = DBService.Instance.Entities.Characters.FirstOrDefault(v => v.ID == targetId);
